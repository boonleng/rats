...
def nn(x):
    ...
    with tf.name_scope('conv1'):
        W_conv1 = weight_variable([3, 1, 1, N_conv1])
    ...
    with tf.name_scope('conv2'):
        W_conv2 = weight_variable([3, 1, N_conv1, N_conv2])
...
for i in range(0, len(data) - kernel_size - N, N):
# for i in [0]:
    # for k in range(N):
    #     xx[k, :, 0] = y_open[:, i + k : i + k + kernel_size]
    #     xx[k, :, 1] = y_close[:, i + k : i + k + kernel_size]
    #     yy[k, :] = ups[i + k + kernel_size : i + k + kernel_size + 1, :]
    if i % 50 == 0:
        # train_accuracy = accuracy.eval(session = sess,
        #                                feed_dict = {x: xx, y_true: yy, keep_prob: 1.0})
        run_options = tf.RunOptions(trace_level = tf.RunOptions.FULL_TRACE)
        run_metadata = tf.RunMetadata()
        summ, train_accuracy = sess.run([merged_summary, accuracy], 
          feed_dict = {x: xx, y_true: yy, keep_prob: 1.0},
